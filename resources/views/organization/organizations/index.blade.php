...
<div class="space-y-5">
    ...
    <div class="flex items-start justify-between gap-4">
        <div class="flex-1">
            <h1 class="text-2xl font-semibold text-white mb-1">Organization Management</h1>
            <p class="text-sm text-slate-400">Manage your organization's structure and locations</p>
        </div>
        <div class="flex-shrink-0">
            <a href="{{ route('organizations.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-lg transition-all shadow-sm">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 lg:flex-1">
                        <div class="flex-1 sm:max-w-[200px]">
                            ...
                            >
                                ...
                                <option value="unit" {{ request('type') == 'unit' ? 'selected' : '' }}>Unit</option>
                ...
                <thead>
                    <tr class="border-b border-slate-700/50 bg-slate-800/30">
                        ...
                        <th class="px-4 py-3.5 text-center">
                            <span class="text-xs font-semibold text-slate-400 uppercase tracking-wide">Depts</span>
                        </th>
                        <th class="px-4 py-3.5 text-center">
                    ...
                    @forelse($organizations as $organization)
                        <tr class="hover:bg-slate-800/30 transition-colors group">
                            <td class="px-4 py-3.5">
                                <span class="text-xs font-mono text-slate-500 bg-slate-800/50 px-2 py-1 rounded">{{ $organization->code }}</span>
                            </td>
                            <td class="px-4 py-3.5">
                                <span class="text-sm font-medium text-white">{{ $organization->name }}</span>
                            </td>
                            <td class="px-4 py-3.5">
                            ...
                            <td class="px-4 py-3.5">
                                <span class="text-sm text-slate-400">{{ $organization->parent?->name ?? '—' }}</span>
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                <span class="text-sm text-slate-300">{{ $organization->departments_count }}</span>
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                <span class="text-sm text-slate-300">{{ $organization->assignments_count }}</span>
                            </td>
                            <td class="px-4 py-3.5">
                            ...
                            <td colspan="8" class="px-4 py-16">
                                <div class="flex flex-col items-center justify-center text-center max-w-sm mx-auto">
                                    <div class="w-14 h-14 rounded-xl bg-slate-800/50 flex items-center justify-center mb-4">
                                        <i data-lucide="building-2" class="w-7 h-7 text-slate-600"></i>
                                    </div>
                                    <h3 class="text-base font-semibold text-white mb-1">No organizations found</h3>
