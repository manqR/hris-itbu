    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">
                {{ isset($menu) ? 'Edit Menu' : 'Create New Menu' }}
            </h1>
            <p class="text-slate-400 mt-1">{{ isset($menu) ? 'Update menu properties' : 'Add a new menu item to the sidebar' }}</p>
        </div>
    </div>

    {{-- Form --}}
    <div class="bg-slate-900/90 backdrop-blur-sm rounded-xl border border-slate-700/50 shadow-lg">
        <form action="{{ isset($menu) ? route('menus.update', $menu) : route('menus.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            @if(isset($menu))
                @method('PUT')
            @endif
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="w-12 h-12 rounded-xl bg-slate-800/50 border border-slate-700/50 flex items-center justify-center" id="iconPreview">
                        <i data-lucide="{{ old('icon', $menu->icon ?? 'circle') }}" class="w-5 h-5 text-slate-400"></i>
                    </div>
                </div>
            ...
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', $menu->order ?? 0) }}" min="0"
